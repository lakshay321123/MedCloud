...
function UsersTab() {
  ...
  async function handleCreateUser() {
    ...
    try {
      ...
      const msg = err?.response?.error || err?.message || 'Unknown error'
        ...
        <>
          ...
          <div className="fixed inset-0 flex items-center justify-center z-50">
            <div className="bg-surface-secondary rounded-xl p-6 w-full max-w-md shadow-2xl space-y-4">
              ...
              <div>
                <label className="text-[13px] text-content-secondary block mb-1">Password {newUser.password ? <span className="text-brand-dark">(will create login)</span> : <span className="text-content-tertiary">(optional — leave blank for DB-only)</span>}</label>
                <input value={newUser.password} onChange={e=>setNewUser(p=>({...p,password:e.target.value}))} placeholder="Min 8 chars, 1 upper, 1 lower, 1 number" type="password" className="w-full bg-surface-elevated border border-separator rounded-lg px-3 py-2 text-sm text-content-secondary focus:outline-none focus:border-brand/40"/>
              ...
              <button
                onClick={async () => {
                  ...
                  try {
                    ...
                    setShowAddOrg(false)
                    // Force page refresh to show new org
                    window.location.reload()
                  } catch (err: any) {
                    toast.error(`Failed: ${err?.response?.error || err?.message || 'Unknown error'}`)
                  }
                }}
