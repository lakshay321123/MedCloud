...
function UsersTab() {
  ...
  const [newUser, setNewUser] = useState({ name: '', email: '', role: 'coder', clientId: '' })
  ...
  async function handleCreateUser() {
    ...
    try {
      ...
      const payload: any = { first_name: nameParts[0] || '', last_name: nameParts.slice(1).join(' ') || '', email: newUser.email, role: newUser.role, is_active: true }
    ...
    setNewUser({ name: '', email: '', role: 'coder', clientId: '' })
    setShowAdd(false)
    setCreating(false)
  }
        ...
        <>
          ...
          <div className="fixed inset-0 flex items-center justify-center z-50">
            <div className="bg-surface-secondary rounded-xl p-6 w-full max-w-md shadow-2xl space-y-4">
              ...
              <p className="text-[11px] text-content-tertiary -mt-2">User will be created as DB-only. Use the <strong>Login</strong> action after creation to enable sign-in.</p>
              <button type="button" onClick={handleCreateUser} disabled={creating}
                className="w-full bg-brand text-white rounded-lg py-2.5 text-sm font-medium hover:bg-brand-deep transition-colors disabled:opacity-50">{creating ? 'Creating…' : 'Create User'}</button>
            </div>
          </div>
        </>
      )}
