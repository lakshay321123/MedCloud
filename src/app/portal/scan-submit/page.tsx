...
export default function ScanSubmitPage() {
  ...
  return (
    <ModuleShell title="Scan & Submit" subtitle="Upload documents to Cosentus for processing" sprint={2}>
          ...
          <div className="bg-bg-secondary border border-border rounded-xl p-4 space-y-4">
            {submitted ? (
              <div className="text-center py-12">
                ...
                <h3 className="text-lg font-semibold text-emerald-400">Received!</h3>
                <p className="text-sm text-muted mt-1">Tracking ID: #SUB-2026-0848</p>
                <p className="text-xs text-muted mt-2">Track progress in Watch & Track</p>
              </div>
            ) : (
              <>
                <div>
                ...
                <div>
                  <label className="text-xs text-muted block mb-1">Upload Files *</label>
                  <div
                    onDragOver={e => e.preventDefault()}
                    onDrop={e => { e.preventDefault(); setFiles(['document.pdf']) }}
                    onClick={() => setFiles(['superbill_upload.pdf'])}
                    className={`border-2 border-dashed rounded-xl py-10 text-center cursor-pointer transition-all ${files.length ? 'border-brand/40 bg-brand/5' : 'border-border hover:border-brand/30'}`}>
                    {files.length ? (
                      ...
                      <div>
                        ...
                        <p className="text-sm text-muted">Drag & drop files or click to browse</p>
                        <p className="text-[10px] text-muted mt-1">PDF, JPG, PNG, TIFF — Max 25MB</p>
                      </div>
                    )}
                  </div>
                </div>
        ...
        <div>
          <div className="bg-bg-secondary border border-border rounded-xl p-4">
            ...
            <div className="space-y-3">{demoSubmissions.filter(s=>s.clientId==='org-102').map(s=>(
              <div key={s.id} className="border-b border-border pb-2 last:border-0 last:pb-0">
                <div className="flex items-center justify-between">
                  <span className="text-xs font-medium">{s.patientName}</span>
                  <StatusBadge status={s.status} small/>
                </div>
                <div className="text-[10px] text-muted mt-0.5">{s.trackingId} • {s.docType}</div>
                <div className="text-[10px] text-muted">{new Date(s.submittedAt).toLocaleDateString()}</div>
              </div>
            ))}</div>
          </div>
        </div>
