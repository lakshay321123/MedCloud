...
function AddPatientModal({ onClose }: { onClose: () => void }) {
  return (
    <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50" onClick={onClose}>
      <div className="card w-[480px] max-h-[80vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
        ...
        <div className="p-4 space-y-4">
          <p className="text-xs text-content-secondary bg-brand/5 border border-brand/20 rounded-lg p-2">Only name and phone are required to start. Fill in the rest when available.</p>
          <div className="grid grid-cols-2 gap-3">
            <div><label className="text-xs text-content-secondary block mb-1">First Name *</label><input className="w-full bg-surface-elevated border border-separator rounded-lg px-3 py-2 text-sm text-content-primary" placeholder="First name"/></div>
            <div><label className="text-xs text-content-secondary block mb-1">Last Name *</label><input className="w-full bg-surface-elevated border border-separator rounded-lg px-3 py-2 text-sm text-content-primary" placeholder="Last name"/></div>
          </div>
          <div><label className="text-xs text-content-secondary block mb-1">Phone *</label><input className="w-full bg-surface-elevated border border-separator rounded-lg px-3 py-2 text-sm text-content-primary" placeholder="Phone number"/></div>
          <div className="border-t border-separator pt-3">
            <p className="text-xs text-content-secondary mb-2">Optional — add when available</p>
            <div className="grid grid-cols-2 gap-3">
              ...
              <div><label className="text-xs text-content-secondary block mb-1">Gender</label>
                <select className="w-full bg-surface-elevated border border-separator rounded-lg px-3 py-2 text-sm text-content-primary"><option value="">Select</option><option>Male</option><option>Female</option><option>Other</option></select>
              </div>
            </div>
            <div className="mt-3"><label className="text-xs text-content-secondary block mb-1">Email</label><input className="w-full bg-surface-elevated border border-separator rounded-lg px-3 py-2 text-sm text-content-primary" placeholder="Email"/></div>
          </div>
          <div className="border-t border-separator pt-3">
            ...
            <button className="w-full bg-surface-elevated border border-dashed border-separator rounded-lg py-3 text-xs text-content-secondary hover:border-brand/30 hover:text-brand transition-all">
              Click to scan Driver&apos;s License / Emirates ID
            </button>
          </div>
          <div className="flex gap-2 pt-2">
            <button onClick={onClose} className="flex-1 bg-surface-elevated border border-separator rounded-lg py-2 text-sm text-content-secondary hover:text-content-primary">Cancel</button>
            <button onClick={onClose} className="flex-1 bg-brand text-white rounded-lg py-2 text-sm font-medium hover:bg-brand-deep">Save Patient</button>
          </div>
        </div>
      </div>
    </div>
...
function PatientDetail({ patient, onClose }: { patient: DemoPatient; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50" onClick={onClose}>
      <div className="card w-[640px] max-h-[85vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
        <div className="flex items-center justify-between p-4 border-b border-separator">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <span className="text-xs text-content-secondary">{patient.id} • {region === 'uae' ? '🇦🇪 UAE' : '🇺🇸 US'}</span>
            </div>
          </div>
        ...
        <div className="flex border-b border-separator">{(['demographics','insurance','documents','visits','messages'] as const).map(t=>(
          <button key={t} onClick={()=>setTab(t)} className={`px-4 py-2 text-xs font-medium border-b-2 transition-all ${tab===t?'border-brand text-brand':'border-transparent text-content-secondary hover:text-content-primary'}`}>{t.charAt(0).toUpperCase()+t.slice(1)}</button>
        ))}</div>
        <div className="p-4 text-sm">
          {tab === 'demographics' && (
            <div className="space-y-3">
              <div className="grid grid-cols-2 gap-3">
                <div><span className="text-xs text-content-secondary block">Name</span><span>{patient.firstName} {patient.lastName}</span></div>
                ...
                {region === 'uae' && <div><span className="text-xs text-content-secondary block">Emirates ID</span><span>{patient.emiratesId || '—'}</span></div>}
                {region === 'us' && <div><span className="text-xs text-content-secondary block">SSN</span><span>{patient.ssn || '—'}</span></div>}
              </div>
              {patient.noShowCount && patient.noShowCount >= 3 && <div className="bg-red-500/10 border border-red-500/20 rounded-lg p-2 text-xs text-red-600 text-red-600 dark:text-red-400">⚠ {patient.noShowCount} no-shows on record</div>}
            </div>
          )}
          {tab === 'insurance' && (
            <div className="space-y-3">
              {patient.insurance ? (
                <div className="bg-surface-elevated border border-separator rounded-lg p-3">
                  ...
                  <div className="grid grid-cols-2 gap-2 text-xs">
                    ...
                    <div><span className="text-content-secondary">Group:</span> {patient.insurance.groupNo || '—'}</div>
                    <div><span className="text-content-secondary">Member ID:</span> {patient.insurance.memberId}</div>
                  </div>
                </div>
              ) : <div className="text-center py-8 text-content-secondary text-xs">No insurance on file. <button className="text-brand underline">Upload insurance card</button></div>}
              ...
              <button className="w-full bg-surface-elevated border border-dashed border-separator rounded-lg py-3 text-xs text-content-secondary hover:border-brand/30">
    ...
    <ModuleShell title="Patients" subtitle="Manage patient records"
      ...
      <div className="mb-4 relative max-w-sm">
        ...
        <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search by name, phone, ID..."
          className="w-full bg-surface-elevated border border-separator rounded-lg pl-9 pr-4 py-2 text-sm text-content-primary placeholder:text-content-tertiary"/>
      </div>
        ...
        <table className="w-full text-sm">
          ...
          <tbody>{patients.map(p=>(
            <tr key={p.id} onClick={()=>setSelected(p)} className="border-b border-separator last:border-0 table-row cursor-pointer transition-all">
              ...
              <td className="px-4 py-3 text-content-secondary text-xs">{p.insurance?.payer || <span className="text-amber-600 text-amber-600 dark:text-amber-400">Not on file</span>}</td>
