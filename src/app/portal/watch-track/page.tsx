...
export default function WatchTrackPage() {
  ...
  return (
    <ModuleShell title="Watch & Track" subtitle="Track your claims and revenue" sprint={2}>
        ...
        <table className="w-full text-sm">
          ...
          <tbody>{filtered.map(c=>(
            <tr key={c.id} className="border-b border-border last:border-0 hover:bg-white/5 cursor-pointer">
              ...
              <td className="px-4 py-3 text-muted text-xs">{c.dos}</td>
              <td className="px-4 py-3 text-right">${c.charges.toLocaleString()}</td>
              <td className="px-4 py-3 text-right text-emerald-400">{c.paid > 0 ? `$${c.paid.toLocaleString()}` : '—'}</td>
