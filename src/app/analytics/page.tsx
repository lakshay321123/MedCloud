...
export default function AnalyticsPage() {
  return (
    <ModuleShell title="Analytics" subtitle="Financial and operational reporting" sprint={5}>
      ...
      <div className="grid grid-cols-2 gap-4">
        <div className="bg-bg-secondary border border-border rounded-xl p-4">
          ...
          <div className="h-48 flex items-end gap-2 px-4">{[1.8,2.0,2.1,2.2,2.3,2.4].map((v,i)=>(
            <div key={i} className="flex-1 flex flex-col items-center gap-1">
              <span className="text-[10px] text-muted">${v}M</span>
        ...
        <div className="bg-bg-secondary border border-border rounded-xl p-4">
          ...
          <div className="space-y-2 text-xs">{[
            ...
            <div key={d.r} className="flex justify-between py-1 border-b border-border last:border-0"><span>{d.r}</span><span className="text-red-400">{d.c}</span></div>
          ))}</div>
        </div>
        <div className="bg-bg-secondary border border-border rounded-xl p-4">
          ...
          <div className="space-y-2 text-xs">{[
            ...
            <div key={c.n} className="flex justify-between py-1.5 border-b border-border last:border-0"><span>{c.n}</span><span className="text-muted">{c.rev}</span><span className="text-emerald-400">{c.rate}</span></div>
          ))}</div>
        </div>
      </div>
    </ModuleShell>
