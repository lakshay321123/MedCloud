    ...
    <ModuleShell title="Eligibility Verification" subtitle="Check insurance coverage and benefits">
        ...
        <>
          <div className="card p-4 mb-4">
            <div className="grid grid-cols-4 gap-3">
              <select value={selectedClientId} onChange={e => { setSelectedClientId(e.target.value); setSelectedPatientId('') }} className="bg-surface-elevated border border-separator rounded-btn px-3 py-2 text-[13px]">
                {demoClients.map(c => <option key={c.id} value={c.id}>{c.name}</option>)}
              </select>
              <select value={selectedPatientId} onChange={e => setSelectedPatientId(e.target.value)} className="bg-surface-elevated border border-separator rounded-btn px-3 py-2 text-[13px]">
            ...
            <div className="grid grid-cols-3 gap-3 mt-3">
              ...
              <button className="bg-brand text-white rounded-btn px-3 py-2 text-[13px] font-medium">Verify Eligibility</button>
            </div>
          ...
          <div className="card overflow-hidden">
            <table className="w-full text-[13px]">
              ...
              <tbody>{demoChecks.map(c => <tr key={c.id} className="table-row border-b border-separator last:border-0">
      ...
      {tab === 'batch' && (
        <div className="card p-4 text-[13px] space-y-3">
          ...
          <div className="flex gap-3 items-center"><input type="date" value={batchDate} onChange={e => setBatchDate(e.target.value)} className="bg-surface-elevated border border-separator rounded-btn px-3 py-2" /><button className="bg-brand text-white rounded-btn px-3 py-2">Run Batch</button></div>
