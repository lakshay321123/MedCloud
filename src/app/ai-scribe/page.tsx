    ...
    <ModuleShell title="AI Scribe" subtitle={isProvider ? 'Dictate and review clinical notes' : 'Review AI-generated clinical notes'}>
      {isProvider && (
        <div className="card p-4 mb-4 flex items-center justify-between">
          <div>
            ...
            <p className="text-xs text-content-secondary">Select a patient and begin recording</p>
          </div>
          <button onClick={() => setRecording(!recording)}
            className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium ${recording ? 'bg-red-500 text-white' : 'bg-brand text-white hover:bg-brand-deep'}`}>
      ...
      <div className="grid grid-cols-3 gap-4 h-[calc(100vh-320px)]">
        ...
        <div className="card overflow-auto">
          <div className="px-3 py-2 border-b border-separator text-xs font-semibold text-content-secondary">
            {isProvider ? 'My Notes' : 'Signed Notes (Read-Only)'}
          </div>
          {notes.map(n => (
            <button key={n.id} onClick={() => setSelectedNote(n)}
              className={`w-full text-left px-3 py-3 border-b border-separator table-row ${selectedNote.id === n.id ? 'bg-brand/5' : ''}`}>
              <div className="flex items-center justify-between">
                <span className="text-sm font-medium">{n.patient}</span>
                <StatusBadge status={n.status === 'signed' ? 'completed' : 'in_progress'} small/>
              </div>
              <div className="text-[10px] text-content-secondary">{n.provider} • {n.date}</div>
            </button>
          ))}
        </div>

        {/* SOAP Note */}
        <div className="col-span-2 card overflow-auto flex flex-col">
          <div className="px-4 py-3 border-b border-separator flex items-center justify-between">
            <div>
              ...
              <span className="text-[10px] text-content-secondary">{selectedNote.provider} • {selectedNote.date}</span>
            </div>
            <div className="flex items-center gap-1 text-xs text-content-secondary"><Play size={12}/> Play recording</div>
          </div>
          <div className="flex-1 overflow-auto p-4 space-y-3">
            {Object.entries(selectedNote.soap).map(([key, val]) => (
              <div key={key}>
                <div className="text-xs font-semibold text-content-secondary mb-1">{key === 's' ? 'SUBJECTIVE' : key === 'o' ? 'OBJECTIVE' : key === 'a' ? 'ASSESSMENT' : 'PLAN'}</div>
                <div className="text-sm bg-surface-elevated rounded-lg p-2">{val}</div>
              </div>
            ))}
            <div className="border-t border-separator pt-3">
              <div className="text-xs font-semibold text-content-secondary mb-2">AI-SUGGESTED CODES</div>
              <div className="flex flex-wrap gap-2">
                ...
                {selectedNote.icd.map(c => <span key={c.code} className="bg-cyan-500/10 text-cyan-600 dark:text-cyan-400 text-xs px-2 py-0.5 rounded border border-cyan-500/20">{c.code} ({c.confidence}%)</span>)}
              </div>
            </div>
          </div>
          {isProvider && selectedNote.status === 'pending_signoff' && (
            <div className="p-3 border-t border-separator">
              <button className="w-full bg-emerald-500/10 text-emerald-600 text-emerald-600 dark:text-emerald-400 border border-emerald-500/20 rounded-lg py-2 text-sm font-medium hover:bg-emerald-500/20 flex items-center justify-center gap-2">
