...
export const handler = async (event) => {
  ...
  try {
    ...
    if (path.includes('/edi-transactions')) {
      if (method === 'GET') {
        try { return respond(200, await list('edi_transactions', effectiveOrgId, clientId, 'ORDER BY created_at DESC')); } catch(e) { if (e.message?.includes('does not exist')) return respond(200, { data: [], meta: { total: 0 } }); throw e; }
      }
      if (method === 'POST') {
