...
CREATE TABLE IF NOT EXISTS ar_call_logs (
  ...
  call_type VARCHAR(20) DEFAULT 'outbound' CHECK (call_type IN ('outbound','inbound','transfer')),
  duration_seconds INT DEFAULT 0,
  outcome VARCHAR(30) DEFAULT 'no_answer' CHECK (outcome IN (
...
-- ═══════════════════════════════════════════════════════════════
ALTER TABLE documents ADD COLUMN IF NOT EXISTS source VARCHAR(50) DEFAULT 'manual_upload';
...
ALTER TABLE denials ADD COLUMN IF NOT EXISTS source_line_item VARCHAR(20);
